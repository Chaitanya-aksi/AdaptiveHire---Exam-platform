...
- **companies** — one business inside a workspace, and the name/logo a candidate actually sees (added 2026-09-08). An *organisation* is the account and the tenancy boundary; a *company* is one of the businesses it hires for, so a group with six subsidiaries has one organisation, one question bank and six companies. `assessments.companyId` is nullable and null means the workspace's own branding, which is what every assessment created before this has. Chosen on the assessment form and changeable afterwards from the assessment's page; the list is managed in Settings → Workspace. **The resolution rule lives in one file, `invitations/candidate-branding.ts`** — company, then organisation, then the platform, resolved *field by field* so a company setting only a name and a logo inherits the group's accent and shared inbox — and it is used by the candidate's list, their attempt record and the rejection/message emails, so what an email is signed by can never disagree with what was on screen. A company id arriving from a client is validated against `@CurrentOrg()` before any write: unvalidated, it would put another customer's logo and support address in front of these candidates. Retiring (`isActive = false`) is the intended way to remove one — it leaves finished attempts naming the business the candidate applied to, which deleting does not.
...
- **Public assessment links (added 2026-09-10, amends the rule above).** A recruiter can enable a **link on an assessment** — `/a/<token>` — that a candidate opens, enters their email on, chooses a password, and goes straight in. One link serves a whole cohort; there is no per-candidate URL. This is an agreed amendment to "NOT a token-based email link", not a departure from it: login and the assessment list both survive, and the token reaches the *sign-up* step rather than replacing authentication. It exists because **invitation email has never been deliverable from production** — Render blocks outbound SMTP on 25/465/587 for free web services and Zoho refuses Render's outbound ranges for OAuth, both proved by credential-free probes rather than guessed. See `docs/public-assessment-links.md` for the full proposal, including what it deliberately gives up.
  - **All of it is additive and inert when unset.** An assessment that never enables a link behaves exactly as it did before, and recruiter invitations are untouched.
  - **No new auth path.** `PublicEntryService` decides only whether the link is open, whether the address may use it, and which of `AuthService.register` / `AuthService.login` to call. Those two remain the only things on the platform that mint a session, and `SessionCookieService` — extracted from `AuthController` for this — remains the only thing that writes the refresh cookie. A second copy of either would be a second place to get argon2, the portal check and cookie attributes wrong, and `clearCookie` given different attributes silently leaves the session in place.
  - **Two steps: email, then password.** The email screen comes first because a domain refusal, a closed round and "you already have an account" are all better said before somebody chooses a password. The known cost is that step one answers "does this address have an account" — accepted deliberately, because a single form leaks the same fact while being unable to say any of the three; the rate limit bounds it, the form shape never could.
  - **An existing account must supply its password. Not negotiable.** Candidate accounts are org-less and shared across every company that invites that person, so admitting somebody on a typed address alone would be account takeover *and* would expose that person's attempts for other employers. `portal: candidate` reuses the existing check, so a recruiter's address is refused here exactly as at `/login`.
  - **Order of writes is a security property, not a detail.** For a new address the invitation is written *before* `register`, because registration is invite-gated and that gate is worth keeping — and it is deleted again if registration fails. For an existing address login runs *first*, so a wrong password cannot leave invitations behind against any address somebody can name, or drain the attempt cap doing it.
  - **Only the token's SHA-256 is stored.** The URL is returned once, at minting, and cannot be looked up again; losing it means minting another, which invalidates the one already shared. Same bargain as `password_reset_tokens`, and minting is therefore also how a leaked link is revoked.
  - **`invitations.source` marks provenance, and both cases are shown.** An invited attempt is evidence about a named person because somebody vouched for the address; a self-registered one is evidence about whoever typed it. It appears on the results list, the report page and the PDF. Labelling only the self-registered ones would read as an accusation against that candidate rather than as the scale the result sits on — the same rule `expectedByChance` follows.
  - **`registeredIp` and `registeredUserAgent` are recorded and shown, never enforced.** Blocking a repeat IP was considered and rejected: a college, an office or a household shares one address and Indian carriers use CGNAT, so it would refuse legitimate candidates in bulk while a VPN defeats it in seconds. Detect and log for human judgment — the same rule the whole proctoring stack runs on.
  - The rate limits on `/public/assessments/*` are **deliberately loose** (60/30/20 a minute). The headline use case is a campus drive where a hundred candidates share one public address, and a limit tuned for one person per IP would have the cohort 429ing each other — the same false-positive shape that got IP blocking rejected.
